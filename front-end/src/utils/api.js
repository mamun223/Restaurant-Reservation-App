...
const API_BASE_URL =
  process.env.REACT_APP_API_BASE_URL || "postgres://wmafhdvs:lR6ch1B5f_rSYGz2QVUgd9z1jpHSiSZX@lallah.db.elephantsql.com/wmafhdvs";
...
export async function updateReservationStatus(reservationId) {
  ...
  const options = {
    method: "PUT",
    headers,
    body: JSON.stringify({data: { reservation_id: reservationId, status: "booked" } }),
  };
  return await fetchJson(url, options, {});
}
